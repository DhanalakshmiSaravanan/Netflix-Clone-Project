...
import {AiOutlineInfoCircle,GrPlayFill} from "../../icons"
import YouTube from "react-youtube"
import {trendingMovies,watchmovie} from "../../lib/axios/api-functions/movies"
...
return (
<div>
  {play?(<header className="header-container"
  ...
  <button className='banner-bt1'  onClick={()=>{navigator(`/watch/${movie.id}`);watchmovie(movie)}}>Play</button>
  <GrPlayFill className='absolute top-[23px] left-[26px] text-[25px]'/>
  <button className='banner-bt2' onClick={() => navigator(`/home/${movie.id}`)} >More info</button>
  ...
  className="bg-white p-2 text-black left-12 top-56 absolute">back</button>
</div>
)}
</div>
)
